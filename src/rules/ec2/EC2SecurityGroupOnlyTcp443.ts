    ...
    if (node instanceof CfnSecurityGroup) {
      ...
      if (ingressRules != undefined) {
        ...
        for (const rule of ingressRules) {     
          ...
          const resolvedcidrIp = NagRules.resolveIfPrimitive(node, Stack.of(node).resolve(rule).cidrIp);
          const resolvedcidrIpv6 = NagRules.resolveIfPrimitive(node, Stack.of(node).resolve(rule).cidrIpv6);
        ...
        if (resolvedcidrIp){
          if (resolvedcidrIp === '0.0.0.0/0'){
            if (toPort === 443 && ipProtocol === 'tcp'){
              console.log(resolvedcidrIp, toPort, ipProtocol)
              return NagRuleCompliance.COMPLIANT
            }
            return NagRuleCompliance.NON_COMPLIANT
          }
